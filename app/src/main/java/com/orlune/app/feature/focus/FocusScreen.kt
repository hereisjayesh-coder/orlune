...
import com.orlune.app.ui.components.FocusDurationPicker
...
fun FocusScreen(
    ...
    val activeSessions = sessions.filter { FocusSessionEngine.stateOf(it, System.currentTimeMillis()) == FocusSessionState.ACTIVE }
    val scheduledSessions = sessions.filter { FocusSessionEngine.stateOf(it, System.currentTimeMillis()) == FocusSessionState.SCHEDULED }
    val activeOrScheduled = activeSessions + scheduledSessions
    ...
    // The soonest-ending active session gets the hero treatment — the one the user is
    // actually about to feel the effect of, when more than one happens to overlap.
    val heroSession = remember(activeSessions) {
        activeSessions.minByOrNull { it.startTs + it.plannedMinutes * 60_000L }
    }
    ...
    LazyColumn(modifier = modifier.fillMaxSize().padding(horizontal = 20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        if (heroSession != null) {
            item {
                ActiveFocusHero(
                    session = heroSession,
                    blockedAppDisplay = blockedAppDisplay,
                    notificationSummary = notificationSummary(heroSession, notificationPolicyAccessGranted),
                    onStop = onStop
                )
            }
        }
        ...
        if (heroSession == null) {
            item { Text("Choose what to pause", style = MaterialTheme.typography.titleLarge) }
            item {
                Row(
                    modifier = Modifier
                        .fillMaxWidth()
            ...
            item { Text("Duration", style = MaterialTheme.typography.titleLarge) }
            item { FocusDurationPicker(minutesText = minutesText, onMinutesChange = onMinutesChange) }
...
/** The prominent "session in progress" visual — a big live countdown plus what's
 * paused and how notifications are handled, replacing the numeric setup form while a
 * session actually runs (per the redesigned Focus screen's "don't overload this
 * screen" goal: setup controls and an active session are never shown at once). */
@Composable
private fun ActiveFocusHero(
    session: FocusSessionEntity,
    blockedAppDisplay: Map<String, AppDisplayInfo>,
    ...
    val plannedEndMillis = session.startTs + session.plannedMinutes * 60_000L
    val remainingSeconds = ((plannedEndMillis - nowMillis) / 1000).coerceAtLeast(0)
    ...
    Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface)) {
        Column(
            ...
            if (remainingSeconds <= 0L) {
                Text("🎯 Focus session complete.", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.SemiBold)
            } else {
                Text("Focus", style = MaterialTheme.typography.labelLarge, color = MaterialTheme.colorScheme.onSurfaceVariant)
                Text(formatCountdown(remainingSeconds), style = MaterialTheme.typography.displayMedium, fontWeight = FontWeight.SemiBold)
                Text("remaining", color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
            Spacer(modifier = Modifier.height(16.dp))
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                blockedPackages.forEach { packageName -> AppIcon(blockedAppDisplay[packageName]?.icon) }
            }
            Text(
            ...
            Text(notificationSummary, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
            Spacer(modifier = Modifier.height(16.dp))
            OutlinedButton(onClick = onStop, modifier = Modifier.fillMaxWidth()) { Text("Stop Focus") }
        }
    }
}
